// v50: reliable iPhone jockey-type selector + v49 tournament flow
'use strict';
...
function allMembersReady(){const members=[...state.members.values()];return members.length>0&&members.every(m=>m.ready&&Number.isFinite(Number(m.ready.selection))&&Number(m.ready.selection)>0&&['win','place'].includes(m.ready.betType)&&Number.isFinite(Number(m.ready.stake))&&Number(m.ready.stake)>=100)}
function renderWaiting(){const box=$('waitingText'),btn=$('hostStartRaceBtn');if(!box||!btn)return;const members=[...state.members.values()],ready=members.filter(m=>m.ready).length,total=members.length;box.innerHTML=`<b>${ready}/${total}人 準備完了</b><br>${members.map(m=>`${m.ready?'✅':'⏳'} ${escapeHtml(m.horse||m.name||'参加者')}：${m.ready?'馬券確定':'選択中'}`).join('<br>')}`;btn.hidden=!state.isHost;btn.disabled=!allMembersReady();btn.textContent=allMembersReady()?'全員準備完了・レース開始':`全員の確定待ち（${ready}/${total}）`}
...
function prepareOnlineRace(){
 ...
 const betSelections=Object.fromEntries([...state.members.values()].filter(m=>m.ready).map(m=>[m.id,{number:Number(m.ready.selection),betType:m.ready.betType,stake:Number(m.ready.stake)}]));
