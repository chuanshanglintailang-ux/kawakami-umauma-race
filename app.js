...
function betDisclosure(m){if(isSpectatorMember(m))return '👀 観戦';if(!m.ready)return '⏳ 選択中';const h=state.field.find(x=>x.number===Number(m.ready.selection));return `✅ ${m.ready.betType==='win'?'単勝':'複勝'} ${m.ready.selection}番${h?` ${escapeHtml(h.name)}`:''}・${Number(m.ready.stake).toLocaleString('ja-JP')}pt`}
function renderLiveSpectatorBets(payload=state.racePayload){const box=$('liveSpectatorBets');if(!box)return;const visible=(state.mode==='host'||state.mode==='join')&&state.screen==='race';box.hidden=!visible;if(!visible){box.innerHTML='';return}const bets=payload?.betSelections||{};const field=payload?.field||state.field||[];const rows=[...state.members.values()].filter(m=>!isSpectatorMember(m)).map(m=>{const b=bets[m.id]||m.ready;if(!b)return `<div class="spectator-bet-chip pending"><b>${escapeHtml(m.horse||m.name||'参加者')}</b><span>買い目なし</span></div>`;const h=field.find(x=>Number(x.number)===Number(b.number||b.selection));const number=Number(b.number||b.selection),type=(b.betType==='place'?'複勝':'単勝'),stake=Number(b.stake)||0;return `<div class="spectator-bet-chip"><b>${escapeHtml(m.horse||m.name||'参加者')}</b><span>${type} ${number}番${h?` ${escapeHtml(h.name)}`:''}</span><small>${stake.toLocaleString('ja-JP')}pt</small></div>`}).join('');box.innerHTML=`<div class="spectator-bet-title"><b>🎫 みんなの買い目</b><small>大会参加者全員に表示中</small></div><div class="spectator-bet-scroll">${rows||'<div class="spectator-bet-chip pending"><span>買い目情報がありません</span></div>'}</div>`}
function renderWaiting(){
 const box=$('waitingText'),btn=$('hostStartRaceBtn');if(!box||!btn)return;
 const members=[...state.members.values()],active=bettingMembers(),ready=active.filter(validReady).length,total=active.length,spectatorView=isLocalSpectator(),discloseView=(state.mode==='host'||state.mode==='join');
 const header=total?`${ready}/${total}人 準備完了`:'投票できる参加者はいません';
 box.innerHTML=`${spectatorView?'<div class="privacy-banner"><b>👀 観戦モード</b><br>所持ポイントが0ptのため馬券操作はできません。ほかの参加者の買い目を見ながら観戦できます。</div>':''}<b>${header}</b><br>${members.map(m=>`${isSpectatorMember(m)?'👀':validReady(m)?'✅':'⏳'} ${escapeHtml(m.horse||m.name||'参加者')}：${discloseView?betDisclosure(m):(isSpectatorMember(m)?'観戦中':validReady(m)?'馬券確定':'選択中')}${m.host?'（主催者）':''}`).join('<br>')}`;
