...
function loadWallet(){const stored=localStorage.getItem('kawakami-wallet');if(stored===null||stored==='')return 10000;const raw=Number(stored);return Number.isFinite(raw)&&raw>=0?Math.floor(raw):10000}
const state={screen:'home',history:[],mode:'party',wallet:loadWallet(),player:null,points:{speed:0,stamina:0,kick:0,start:0,mud:0},raceNo:1,venue:'東京',race:RACES[9],going:'良',fieldSize:10,field:[],betType:'win',selection:null,stake:500,result:[],seed:Date.now(),peer:null,hostConn:null,connections:new Map(),members:new Map(),roomCode:'',hostPeerId:'',isHost:false,racePayload:null,replay:false,selectedJockeyId:'take-yutaka',raceRuntime:null,partyEntries:[],partyIndex:0,partyPoints:{speed:0,stamina:0,kick:0,start:0,mud:0},partyEditing:false,navToken:0,tournament:{total:1,current:1,schedule:[],scores:{},rounds:[]},roundSettled:false,myRunnerNumber:null,playerAssignments:{},confirmedBet:null};
...
function saveWallet(){state.wallet=Math.max(0,Math.floor(Number(state.wallet)||0));localStorage.setItem('kawakami-wallet',String(state.wallet));$('wallet').textContent=state.wallet.toLocaleString('ja-JP')}
function syncLocalWalletFromTournament(){const id=state.player?.id;if(!id)return;const score=Number(state.tournament?.scores?.[id]);if(Number.isFinite(score)&&score>=0){state.wallet=Math.floor(score);saveWallet()}}
function resetWalletAfterTournament(){state.wallet=10000;saveWallet()}
function memberPoints(m){const v=Number(state.tournament?.scores?.[m?.id]??m?.tournamentPoints??m?.wallet??10000);return Number.isFinite(v)?Math.max(0,Math.floor(v)):10000}
function isSpectatorMember(m){return memberPoints(m)<=0}
function isLocalSpectator(){const m=state.members.get(state.player?.id);return !!m&&isSpectatorMember(m)}
function bettingMembers(){return [...state.members.values()].filter(m=>!isSpectatorMember(m))}
function validReady(m){return !!(m?.ready&&Number.isFinite(Number(m.ready.selection))&&Number(m.ready.selection)>0&&['win','place'].includes(m.ready.betType)&&Number.isFinite(Number(m.ready.stake))&&Number(m.ready.stake)>=100&&Number(m.ready.stake)<=memberPoints(m))}
function allMembersReady(){const active=bettingMembers();return active.every(validReady)}
function betDisclosure(m){
 if(isSpectatorMember(m))return '👀 観戦';
 if(!m.ready)return '⏳ 選択中';
 const h=state.field.find(x=>x.number===Number(m.ready.selection));
 return `✅ ${m.ready.betType==='win'?'単勝':'複勝'} ${m.ready.selection}番${h?` ${escapeHtml(h.name)}`:''}`
}
function betRowsHtml({payload=null,includePending=true}={}){
 const bets=payload?.betSelections||{};
 const field=payload?.field||state.field||[];
 return [...state.members.values()].map(m=>{
  if(isSpectatorMember(m))return '';
  const b=bets[m.id]||m.ready;
  const displayName=escapeHtml(m.horse||m.name||'参加者');
  if(!b)return includePending?`<div class="spectator-bet-chip pending"><b>${displayName}${m.host?' 👑':''}</b><span>選択中</span></div>`:'';
  const number=Number(b.number||b.selection);
  const h=field.find(x=>Number(x.number)===number);
  const type=b.betType==='place'?'複勝':'単勝';
  return `<div class="spectator-bet-chip"><b>${displayName}${m.host?' 👑':''}</b><span>${type} ${number}番${h?` ${escapeHtml(h.name)}`:''}</span></div>`
 }).join('')
}
function renderWaitingBets(){
 const box=$('waitingBets');if(!box)return;
 const online=state.mode==='host'||state.mode==='join';
 box.hidden=!online;
 if(!online){box.innerHTML='';return}
 const rows=betRowsHtml({includePending:true});
 box.innerHTML=`<div class="spectator-bet-title"><b>🎫 みんなの買い目</b><small>主催者・参加者の全員に表示</small></div><div class="spectator-bet-scroll">${rows||'<div class="spectator-bet-chip pending"><span>買い目情報がありません</span></div>'}</div>`
}
function renderLiveSpectatorBets(payload=state.racePayload){
 const box=$('liveSpectatorBets');if(!box)return;
 const visible=(state.mode==='host'||state.mode==='join');
 box.hidden=!visible;
 if(!visible){box.innerHTML='';return}
 const rows=betRowsHtml({payload,includePending:false});
 box.innerHTML=`<div class="spectator-bet-title"><b>🎫 レース中のみんなの買い目</b><small>単勝・複勝のみ表示</small></div><div class="spectator-bet-scroll">${rows||'<div class="spectator-bet-chip pending"><span>買い目情報がありません</span></div>'}</div>`
}
function renderWaiting(){
 const box=$('waitingText'),btn=$('hostStartRaceBtn');if(!box||!btn)return;
 const members=[...state.members.values()],active=bettingMembers(),ready=active.filter(validReady).length,total=active.length,spectatorView=isLocalSpectator();
 const header=total?`${ready}/${total}人 準備完了`:'投票できる参加者はいません';
 box.innerHTML=`${spectatorView?'<div class="privacy-banner"><b>👀 観戦モード</b><br>所持ポイントが0ptのため馬券操作はできません。</div>':''}<b>${header}</b><br>${members.map(m=>`${isSpectatorMember(m)?'👀':validReady(m)?'✅':'⏳'} ${escapeHtml(m.horse||m.name||'参加者')}：${isSpectatorMember(m)?'観戦中':validReady(m)?'馬券確定':'選択中'}${m.host?'（主催者）':''}`).join('<br>')}`;
 renderWaitingBets();
 btn.hidden=!state.isHost;btn.disabled=!allMembersReady();btn.textContent=allMembersReady()?(total?'全員準備完了・レース開始':'観戦レースを開始'):`全員の確定待ち（${ready}/${total}）`;
}
...
function ensureCustomJockeyTypes(){const sel=$('customJockeyType');if(!sel)return;const current=sel.value;const types=Object.keys(CUSTOM_JOCKEY_TYPES);const existing=[...sel.options].map(o=>o.value);if(sel.options.length!==types.length||types.some((x,i)=>existing[i]!==x)){sel.replaceChildren(...types.map(x=>{const o=document.createElement('option');o.value=x;o.textContent=x;return o}))}sel.value=types.includes(current)?current:'バランス型';sel.disabled=false;sel.removeAttribute('aria-disabled')}
function setupSelects(){for(const id of ['partyVenue','partyNextVenue'])$(id).innerHTML=Object.keys(VENUES).map(v=>`<option>${v}</option>`).join('');for(const id of ['partyRace','partyNextRace'])$(id).innerHTML=RACES.map((r,i)=>`<option value="${i}" ${i===9?'selected':''}>${r.name}（${r.distance}m）</option>`).join('');$('partyVenue').value='東京';ensureCustomJockeyTypes();$('partyFamousHorse').innerHTML=LEGENDS.map((h,i)=>`<option value="${i}">${escapeHtml(h.name)}（${h.style}）</option>`).join('');$('partyFamousJockey').innerHTML=JOCKEYS.map(j=>`<option value="${j.id}">${escapeHtml(j.name)}（${j.era}・${j.type}）</option>`).join('');renderHostSchedule()}
...
function createPlayer(){ensureCustomJockeyTypes();const horse=$('horseName').value.trim(),used=Object.values(state.points).reduce((a,b)=>a+b,0);if(!horse){$('createError').textContent='馬の名前を入力してな。';return null}if(used!==10){$('createError').textContent='追加10ポイントを全部使い切ってな。';return null}$('createError').textContent='';const jockeyName=$('customJockeyName').value.trim();if(!jockeyName){$('createError').textContent='オンライン騎手の名前も入力してな。';return null}const jt=$('customJockeyType').value;if(!CUSTOM_JOCKEY_TYPES[jt]){$('createError').textContent='騎手タイプを選択してな。';ensureCustomJockeyTypes();return null}return{id:crypto.randomUUID?crypto.randomUUID():String(Date.now())+Math.random(),name:'オンライン参加者',horse,style:$('horseStyle').value,abilities:Object.fromEntries(ABILITIES.map(k=>[k,90+state.points[k]])),jockey:{id:'custom-'+Date.now(),name:jockeyName,era:'オリジナル',type:jt,bonus:{...CUSTOM_JOCKEY_TYPES[jt]}},wallet:state.wallet}}
function beginCreate(mode){state.mode=mode;state.points={speed:0,stamina:0,kick:0,start:0,mud:0};$('createTitle').textContent=mode==='host'?'主催する馬を作る':'参加する馬を作る';ensureCustomJockeyTypes();renderAbility();show('horseCreate');requestAnimationFrame(ensureCustomJockeyTypes)}
...
function calculateProbabilities(){
 const scores=state.field.map(baseScore),max=Math.max(...scores),n=Math.max(1,state.field.length);
 // 能力差を人気へ反映しつつ、15%は全馬に均等配分して穴馬の勝機を残す。
 const raw=scores.map(s=>Math.exp((s-max)/6.8)),rawSum=raw.reduce((a,b)=>a+b,0)||1;
 const probs=raw.map(w=>.85*(w/rawSum)+.15/n),sum=probs.reduce((a,b)=>a+b,0)||1;
 state.field.forEach((h,i)=>{h.power=scores[i];h.winProb=probs[i]/sum;h.placeProb=Math.min(.88,h.winProb*2.15+.09);h.winOdds=Math.max(1.2,Math.min(99.9,.82/h.winProb));h.placeOdds=Math.max(1.1,Math.min(20,.78/h.placeProb))});
 [...state.field].sort((a,b)=>a.winOdds-b.winOdds).forEach((h,i)=>h.popularity=i+1)
}
...
async function startFromBet(){
 updateTicket();if($('raceStartBtn').disabled)return;
 const selected=state.field.find(h=>h.number===Number(state.selection));if(!selected)return;
 // 馬券確定時点の馬番を独立したスナップショットとして保存する。
 // 以後、画面遷移・ルーム同期・枠順情報で上書きしない。
 state.confirmedBet={number:selected.number,betType:state.betType,stake:state.stake,name:selected.name,seed:state.seed};
 await unlockAudio();
 if(state.mode!=='host'&&state.mode!=='join'){state.wallet-=state.stake;saveWallet();startCountdown({seed:state.seed,field:state.field,startAt:Date.now()+3300,localBet:{...state.confirmedBet}});return}
 const ready={selection:state.confirmedBet.number,betType:state.confirmedBet.betType,stake:state.confirmedBet.stake};
 if(state.mode==='join')sendHost({type:'ready',playerId:state.player.id,...ready});
 else {const mine=state.members.get(state.player.id);if(mine)mine.ready=ready;broadcastMembers();renderLobby()}
 show('waiting');renderWaiting()
}
function createRoom(){$('hostError').textContent='';const schedule=readHostSchedule();if(!schedule.length){$('hostError').textContent='開催レースを1つ以上選んでな。';return}resetWalletAfterTournament();Object.assign(state,chosenRace('host'));state.raceNo=1;state.tournament={total:schedule.length,current:1,schedule:schedule.map(x=>({venue:x.venue,race:x.race})),scores:{[state.player.id]:10000},rounds:[]};state.roundSettled=false;state.roomCode=makeRoomToken();state.isHost=true;state.mode='host';state.members=new Map([[state.player.id,{...state.player,wallet:10000,ready:null,host:true,tournamentPoints:10000}]]);openHostPeer()}
...
function hostReceive(conn,data){if(!data||!data.type)return;if(data.type==='join'){const p=data.player;if(!p||!p.id||data.invite!==state.roomCode)return;state.members.set(p.id,{...p,wallet:10000,ready:null,connPeer:conn.peer,tournamentPoints:10000});state.tournament.scores[p.id]=10000;conn.playerId=p.id;broadcastMembers();renderLobby();renderWaiting()}if(data.type==='ready'){const m=state.members.get(data.playerId),selection=Number(data.selection),stake=Number(data.stake),betType=data.betType;if(m&&!isSpectatorMember(m)&&state.field.some(h=>h.number===selection)&&['win','place'].includes(betType)&&Number.isFinite(stake)&&stake>=100&&stake<=memberPoints(m)){m.ready={selection,betType,stake};broadcastMembers();renderLobby();renderWaiting()}}if(data.type==='profileUpdate'){const m=state.members.get(data.playerId);if(m){m.name=data.name;m.horse=data.horse;m.ready=null;if(state.player?.id===m.id)state.player={...state.player,name:data.name,horse:data.horse};broadcastMembers();renderLobby();renderWaiting()}}}
function joinReceive(data){if(!data||!data.type)return;if(data.type==='hello'){Object.assign(state,data.config);state.members=new Map(data.members.map(m=>[m.id,m]));state.wallet=10000;saveWallet();$('lobbyStatus').textContent='ルームに参加しました。';renderLobby();renderWaiting()}if(data.type==='members'){state.members=new Map(data.members.map(m=>[m.id,m]));renderLobby();renderWaiting();if((state.screen==='bet'||state.screen==='waiting')&&isLocalSpectator()){show('waiting',false);renderWaiting()}}if(data.type==='goBet'){Object.assign(state,data.config);state.seed=data.seed;state.field=data.field;state.playerAssignments=data.playerAssignments||Object.fromEntries(state.field.filter(h=>h.playerId).map(h=>[h.playerId,h.number]));state.myRunnerNumber=state.playerAssignments[state.player?.id]??state.field.find(h=>h.playerId===state.player?.id)?.number??null;state.selection=null;state.confirmedBet=null;state.betType='win';if(isLocalSpectator()){show('waiting');renderWaiting()}else renderBet()}if(data.type==='startRace'){state.racePayload=data.payload;state.playerAssignments=data.payload.playerAssignments||state.playerAssignments||{};state.myRunnerNumber=state.playerAssignments[state.player?.id]??state.myRunnerNumber;const mine=data.payload.betSelections?.[state.player?.id];if(mine){const h=(data.payload.field||state.field).find(x=>x.number===Number(mine.number));state.confirmedBet={number:Number(mine.number),betType:mine.betType,stake:Number(mine.stake),name:h?.name||'',seed:data.payload.seed};state.selection=Number(mine.number);state.betType=mine.betType;state.stake=Number(mine.stake)}startCountdown(data.payload)}if(data.type==='roundStanding'){state.tournament=data.tournament;syncLocalWalletFromTournament();renderTournamentRanking()}if(data.type==='nextRound'){Object.assign(state,data.config);state.members=new Map(data.members.map(m=>[m.id,m]));state.selection=null;state.confirmedBet=null;state.result=[];renderLobby();renderWaiting();show('lobby')}if(data.type==='finalStanding'){state.tournament=data.tournament;showTournamentFinal();resetWalletAfterTournament()}if(data.type==='roomClosed'){$('lobbyStatus').textContent='主催者がルームを終了しました。'}}
...
function broadcastMembers(){broadcast({type:'members',members:[...state.members.values()]});renderWaiting()}
function removeByConn(conn){if(conn.playerId)state.members.delete(conn.playerId);state.connections.delete(conn.peer);broadcastMembers();renderLobby();renderWaiting()}
function renderLobby(){if(!state.roomCode)return;const t=state.tournament||{current:1,total:1};const showInvite=!!state.isHost;if($('inviteHeading'))$('inviteHeading').hidden=!showInvite;if($('qrBox'))$('qrBox').hidden=!showInvite;$('tournamentProgress').innerHTML=`<div><small>TOURNAMENT</small>第${t.current||1}レース / 全${t.total||1}レース</div><div>${state.isHost?'主催中':'参加中'}</div>`;$('lobbyRaceTitle').textContent=state.race?state.race.name:'レース情報を待っています';$('lobbyConditions').textContent=state.race?conditionText():'';$('lobbyMembers').innerHTML=[...state.members.values()].map((m,i)=>`<div class="member"><span class="badge">${i+1}</span><div><b>${escapeHtml(m.horse)}</b><small>${m.style}・騎手 ${escapeHtml(m.jockey?.name||'未設定')}${m.host?'・主催者':''}</small></div><span><b>${Number(state.tournament?.scores?.[m.id]??m.tournamentPoints??10000).toLocaleString('ja-JP')}pt</b><small>${isSpectatorMember(m)?'👀 観戦モード':m.ready?'✅ 馬券確定':'待機中'}</small></span></div>`).join('')||'<p class="hint">参加者情報を待っています。</p>';$('startOnlineBtn').hidden=!state.isHost;renderWaiting()}
function renderQr(){const box=$('qr'),url=inviteUrl();if(!box)return;box.innerHTML='';box.classList.remove('qr-ready','qr-error');if(window.QRCode){try{new QRCode(box,{text:url,width:154,height:154,correctLevel:QRCode.CorrectLevel.H});box.classList.add('qr-ready')}catch(e){box.classList.add('qr-error');box.innerHTML='<span>QRコードを作成できませんでした。<br>共有ボタンを使ってください。</span>'}}else{box.classList.add('qr-error');box.innerHTML='<span>QRコードの読込に失敗しました。<br>共有ボタンを使ってください。</span>'}$('shareBtn').dataset.url=url}
function hostGoBet(){state.members.forEach(m=>m.ready=null);broadcastMembers();state.seed=Date.now();const scheduled=state.tournament.schedule[(state.tournament.current||1)-1];if(scheduled){state.venue=scheduled.venue;state.race=scheduled.race;state.raceNo=state.tournament.current}const players=shuffled([...state.members.values()],seeded(state.seed+77));makeField(players);state.playerAssignments=Object.fromEntries(state.field.filter(h=>h.playerId).map(h=>[h.playerId,h.number]));state.myRunnerNumber=state.playerAssignments[state.player?.id]??null;const data={type:'goBet',config:raceConfig(),seed:state.seed,field:state.field,playerAssignments:state.playerAssignments};broadcast(data);state.selection=null;state.confirmedBet=null;state.betType='win';state.roundSettled=false;if(isLocalSpectator()){show('waiting');renderWaiting()}else renderBet()}
function prepareOnlineRace(){
 if(!state.isHost)return;if(!allMembersReady()){alert('まだ全員の馬券が確定していません。全員が準備完了になるまで開始できません。');renderWaiting();return}
 const betSelections=Object.fromEntries(bettingMembers().filter(validReady).map(m=>[m.id,{number:Number(m.ready.selection),betType:m.ready.betType,stake:Number(m.ready.stake)}]));
 const payload={seed:state.seed,field:state.field,startAt:Date.now()+4000,config:raceConfig(),playerAssignments:{...state.playerAssignments},betSelections};
 const mine=betSelections[state.player?.id];if(mine){const h=state.field.find(x=>x.number===mine.number);state.confirmedBet={...mine,name:h?.name||'',seed:state.seed};}
 broadcast({type:'startRace',payload});startCountdown(payload)
}
...
const fanfareMedia=new Audio('fanfare.wav');
const gateBangMedia=new Audio('gate-bang.wav');
...
async function playRaceFanfare(raceName){
 try{
  await primeMediaAudio();
  fanfareMedia.pause();fanfareMedia.currentTime=0;fanfareMedia.volume=1;
  const ended=new Promise(resolve=>{const done=()=>{fanfareMedia.removeEventListener('ended',done);resolve(true)};fanfareMedia.addEventListener('ended',done,{once:true});setTimeout(()=>resolve(false),7600)});
  const playPromise=fanfareMedia.play();if(playPromise&&playPromise.then)await playPromise;
  return await ended
 }catch(e){console.warn('wav fanfare failed, using synth fallback',e);try{return await playOriginalFanfare(raceName)}catch(_){return false}}
}

function playGateBangSynth(){
...
async function playGateBang(){
 try{
  await primeMediaAudio();gateBangMedia.pause();gateBangMedia.currentTime=0;gateBangMedia.volume=1;
  const p=gateBangMedia.play();if(p&&p.then)await p;return true
 }catch(e){console.warn('wav gate bang failed, using synth fallback',e);try{playGateBangSynth();return true}catch(_){return false}}
}
function getRaceBackground(canvas,runners){
 const key=`${canvas.width}x${canvas.height}:${runners.length}`;
 if(canvas._raceBg?.key===key)return canvas._raceBg.canvas;
 ...
 const sky=c.createLinearGradient(0,0,0,h*.45);sky.addColorStop(0,'#82c9ff');sky.addColorStop(1,'#e8f5ff');c.fillStyle=sky;c.fillRect(0,0,w,h*.35);
 c.fillStyle='#1f7c42';c.fillRect(0,h*.35,w,h*.65);c.fillStyle='#c69a61';c.fillRect(0,h*.48,w,h*.47);
 c.strokeStyle='#ffffff88';c.lineWidth=2;for(let i=0;i<=runners.length;i++){const y=h*.48+i*(h*.47/runners.length);c.beginPath();c.moveTo(0,y);c.lineTo(w,y);c.stroke()}
 const start=w*.10,goal=w*.93;c.fillStyle='#ffe34f';c.fillRect(start-5,h*.42,10,h*.55);
 for(let y=h*.42;y<h*.97;y+=20){c.fillStyle=((y/20)|0)%2?'#fff':'#111';c.fillRect(goal-7,y,14,20)}
 c.fillStyle='#103d22';c.fillRect(0,h*.95,w,h*.05);c.fillStyle='#fff';c.font=`700 ${Math.max(16,w*.018)}px sans-serif`;c.fillText('START',start-30,h*.44);c.fillText('GOAL',goal-28,h*.44);
 canvas._raceBg={key,canvas:bg};return bg
}
function drawRaceScene(ctx,runners,{gateOpen=0,showGate=false,animTime=performance.now(),mobile=matchMedia('(max-width: 700px)').matches}={}){
 const w=ctx.canvas.width,h=ctx.canvas.height;ctx.clearRect(0,0,w,h);ctx.drawImage(getRaceBackground(ctx.canvas,runners),0,0);
 const start=w*.10,goal=w*.93,trackW=goal-start,horseScale=Math.max(.75,Math.min(1.25,w/1050));
 const gateHorseX=start-(38*horseScale);
 runners.forEach((r,i)=>{const lane=Number.isFinite(r.lane)?r.lane:i;const laneY=h*.48+(lane+.5)*(h*.47/runners.length);const x=gateHorseX+trackW*Math.max(0,r.display||0);drawHorse(ctx,x,laneY,r,lane,animTime,mobile)});
 if(showGate){
...
async function startCountdown(payload){
 const token=++state.navToken;state.racePayload=payload;state.seed=payload.seed;state.field=payload.field;Object.assign(state,payload.config||{});if(payload.playerAssignments){state.playerAssignments=payload.playerAssignments;state.myRunnerNumber=payload.playerAssignments[state.player?.id]??state.myRunnerNumber}const payloadBet=payload.betSelections?.[state.player?.id]||payload.localBet;if(payloadBet&&Number.isFinite(Number(payloadBet.number))){const h=state.field.find(x=>Number(x.number)===Number(payloadBet.number));state.confirmedBet={number:Number(payloadBet.number),betType:payloadBet.betType||state.betType,stake:Number(payloadBet.stake)||state.stake,name:h?.name||'',seed:payload.seed};state.selection=state.confirmedBet.number;state.betType=state.confirmedBet.betType;state.stake=state.confirmedBet.stake}show('race');
 $('liveTitle').textContent=state.race.name;$('liveCond').textContent=conditionText();$('remaining').textContent=`${state.race.distance}m`;$('commentary').textContent='全馬ゲートイン。ファンファーレ演奏を開始します。';$('leaderText').innerHTML='<span>全馬ゲート内で待機中</span>';$('clock').textContent='0:00.0';renderLiveSpectatorBets(payload);
 ...
 try{await unlockAudio();await playGateBang()}catch(e){console.warn('gate sound error',e)}
 previewActive=false;raceRedraw=null;if(audioKeepAlive){try{audioKeepAlive.keep.stop()}catch(e){}audioKeepAlive=null}runRace(payload,token)
}
function officialPlan(field,seed){
 const rnd=seeded(seed^0x9e3779b9);
 // 低オッズ（人気馬）ほど勝ちやすいが、Gumbelノイズで穴馬の激走も十分に起こる。
 // オッズ順をそのまま着順にせず、調子・位置取り・スタートを毎レース別々に抽選する。
 const rows=field.map((h,gateIndex)=>{
  const market=Math.max(.006,Number(h.winProb)||1/Math.max(1,field.length));
  const gumbel=-Math.log(-Math.log(Math.max(1e-9,Math.min(1-1e-9,rnd()))));
  const condition=(rnd()+rnd()+rnd()-1.5)*.34;
  const tactics=(rnd()-.5)*.42;
  const breakLuck=(rnd()-.5)*.24;
  const score=Math.log(market)*.92+gumbel*1.12+condition+tactics+breakLuck+rnd()*1e-5;
  return {h,gateIndex,score}
 }).sort((a,b)=>b.score-a.score);
 const winnerTime=23.78+rnd()*.28;
...
function getConfirmedBet(){
 const remote=state.racePayload?.betSelections?.[state.player?.id];
 if(remote&&Number.isFinite(Number(remote.number)))return {number:Number(remote.number),betType:remote.betType||'win',stake:Number(remote.stake)||0};
 if(state.confirmedBet&&Number.isFinite(Number(state.confirmedBet.number)))return state.confirmedBet;
 if(Number.isFinite(Number(state.selection))&&Number(state.selection)>0)return {number:Number(state.selection),betType:state.betType,stake:state.stake};
 return null
}
function selectedBetRunner(runners){
 const bet=getConfirmedBet(),number=Number(bet?.number);
 if(!Number.isFinite(number)||number<=0)return null;
 return (runners||[]).find(r=>Number(r.number)===number)||state.field.find(r=>Number(r.number)===number)||null
}
...
function runRace(payload,token=state.navToken){
 const canvas=$('track'),ctx=canvas.getContext('2d',{alpha:false,desynchronized:true});resizeCanvas(canvas);
 const plan=officialPlan(payload.field,payload.seed);
 const laneByNumber=new Map(payload.field.map((h,i)=>[h.number,i]));
 const runners=plan.map((h,i)=>({...h,display:0,finish:false,lane:laneByNumber.get(h.number)??h.gateIndex??i,actualFinish:null,lastDisplay:0,visualSpeed:1/RACE_DURATION}));
 buildRaceTimeline(runners,payload.seed,state.race.distance);
 state.raceRuntime={runners,wall:0};
 ...
 function updatePhysics(wall,dt){
  for(let i=0;i<runners.length;i++){
   const h=runners[i],sampled=sampleTimeline(h,wall),ahead=sampleTimeline(h,wall+.14);
   const curveSpeed=Math.max(.016,(ahead-sampled)/.14),chase=(sampled-h.display)*1.9;
   const desired=Math.max(.016,Math.min(.090,curveSpeed+chase)),accel=desired>h.visualSpeed?3.8:2.6;
   h.visualSpeed+=(desired-h.visualSpeed)*Math.min(1,accel*dt);
   h.visualSpeed=Math.max(.016,Math.min(.090,h.visualSpeed));h.lastDisplay=h.display;h.display+=h.visualSpeed*dt;
 ...
 function frame(ts){
  if(token!==state.navToken)return;
  if(sequenceStart===null){sequenceStart=ts;lastFrame=ts;lastRender=ts-renderInterval}
  const rawDt=Math.min(.08,Math.max(0,(ts-lastFrame)/1000));lastFrame=ts;
  ...
  if(ts-lastUi>150){
   $('clock').textContent=`0:${Math.min(59.9,wall).toFixed(1).padStart(4,'0')}`;$('remaining').textContent=`${Math.max(0,Math.round(state.race.distance*(1-Math.min(1,lead.display))))}m`;
   const leaderText=`${lead.number}:${lead.name}`;if(leaderText!==lastLeaderText){const leaderColor=COLORS[(lead.number-1)%COLORS.length];$('leaderText').innerHTML=`<span class="leader-num" style="background:${leaderColor};color:${lead.number===2?'#fff':'#111'}">${lead.number}</span><span>先頭 ${escapeHtml(lead.name)}</span>`;lastLeaderText=leaderText}lastUi=ts
  }
  const call=Math.floor(wall/2.8),remaining=Math.max(0,state.race.distance*(1-Math.min(1,lead.display))),leaderChanged=lastLeader!==null&&lastLeader!==lead.number;lastLeader=lead.number;
  if(call!==lastCall||leaderChanged){lastCall=call;if(remaining<=55)$('commentary').textContent=`ゴール目前！ ${order[0].number}番と${order[1]?.number||order[0].number}番が僅差、わずかに${order[0].number}番が前！`;else if(remaining<=300)$('commentary').textContent=`残り300m！ ${order[0].name}、${order[1]?.name||''}、${order[2]?.name||''}が走りながら激しく抜きつ抜かれつ！`;else if(remaining<=700)$('commentary').textContent=`残り700m！ 馬群が徐々に詰まり、${lead.name}を各馬が追う！`;else if(leaderChanged)$('commentary').textContent=`${lead.number}番${lead.name}が前へ！ 先頭が入れ替わった！`;else $('commentary').textContent=commentary(call,order)}
  let allFinished=true;for(let i=0;i<runners.length;i++){if(!runners[i].finish){allFinished=false;break}}
  if(allFinished||wall>lastFinish+1){if(!done){done=true;order.sort((a,b)=>(a.actualFinish??a.finishWall)-(b.actualFinish??b.finishWall)||a.number-b.number);const base=estimateSeconds(state.race.distance),first=(order[0].actualFinish??order[0].finishWall);state.result=order.map(h=>({...h,officialTime:base+((h.actualFinish??h.finishWall)-first)}));state.raceRuntime=null;raceRedraw=null;setTimeout(showResult,900)}return}
...
function showResult(){show('result');const result=state.result;$('resultList').innerHTML=result.map((h,i)=>`<div class="result-row"><span class="place">${i+1}着</span><span class="horse-num" style="background:${COLORS[(h.number-1)%COLORS.length]};color:${h.number===2?'white':'#111'}">${h.number}</span><span><b>${escapeHtml(h.name)}</b><small>${escapeHtml(h.owner)}・${h.style}・騎手 ${escapeHtml(h.jockey?.name||'未設定')}</small></span><span class="time">${formatTime(h.officialTime)}${i?` +${(h.officialTime-result[0].officialTime).toFixed(2)}`:''}</span></div>`).join('');$('roundRanking').hidden=true;
 ...
 if(state.mode==='host'||state.mode==='join'){$('payoutBox').classList.toggle('win',hit);$('payoutBox').innerHTML=selected?`<b>${betType==='win'?'単勝':'複勝'} ${selected.number}番 ${escapeHtml(selected.name)}</b><br>${hit?`🎉 的中！ 払戻 ${pay.toLocaleString('ja-JP')}pt`:'不的中'}<br><small>現在の所持ポイント ${Number(state.tournament?.scores?.[state.player?.id]??state.wallet).toLocaleString('ja-JP')}pt</small>`:`<b>👀 観戦モード</b><br>このレースは馬券なしで観戦しました。<br><small>現在の所持ポイント 0pt</small>`;if(state.isHost)settleTournament(result);else renderTournamentRanking();const final=(state.tournament.current>=state.tournament.total);$('nextBtn').textContent=final?'総合結果発表へ':'次のレースへ';$('nextBtn').hidden=state.mode==='join';$('replayBtn').hidden=state.mode==='join';if(state.mode==='join')$('payoutBox').innerHTML+=`<br><small>主催者が次のレースへ進むと自動で切り替わります。</small>`;return}
 if(pay){state.wallet+=pay;saveWallet()}$('payoutBox').classList.toggle('win',hit);$('payoutBox').innerHTML=selected?`<b>${betType==='win'?'単勝':'複勝'} ${selected.number}番 ${escapeHtml(selected.name)}</b><br>${hit?`🎉 的中！ 払戻 ${pay.toLocaleString('ja-JP')}pt`:'不的中'}<br><small>現在の所持ポイント ${state.wallet.toLocaleString('ja-JP')}pt</small>`:'馬券なし'}
