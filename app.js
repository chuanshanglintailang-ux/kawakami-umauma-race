// v48: persistent wallet settlement + iPhone smooth race engine + reliable WAV audio, no whip
'use strict';
...
async function startFromBet(){
 ...
 await unlockAudio();
 if(state.mode!=='host'&&state.mode!=='join'){state.wallet-=state.stake;saveWallet();}
 if(state.mode==='join'){
  sendHost({type:'ready',playerId:state.player.id,selection:state.confirmedBet.number,betType:state.confirmedBet.betType,stake:state.confirmedBet.stake});show('waiting')
 }else if(state.mode==='host'){
  state.members.get(state.player.id).ready={selection:state.confirmedBet.number,betType:state.confirmedBet.betType,stake:state.confirmedBet.stake};broadcastMembers();
  const waiting=[...state.members.values()].filter(m=>!m.ready);
  if(waiting.length){alert(`まだ${waiting.length}人が馬券を確定していません。全員の準備完了後にもう一度押してな。`);state.members.get(state.player.id).ready=null;broadcastMembers();return}
  prepareOnlineRace()
 }else startCountdown({seed:state.seed,field:state.field,startAt:Date.now()+3300,localBet:{...state.confirmedBet}})
}
function createRoom(){$('hostError').textContent='';const schedule=readHostSchedule();if(!schedule.length){$('hostError').textContent='開催レースを1つ以上選んでな。';return}Object.assign(state,chosenRace('host'));state.raceNo=1;state.tournament={total:schedule.length,current:1,schedule:schedule.map(x=>({venue:x.venue,race:x.race})),scores:{[state.player.id]:state.wallet},rounds:[]};state.roundSettled=false;state.roomCode=makeRoomToken();state.isHost=true;state.mode='host';state.members=new Map([[state.player.id,{...state.player,ready:null,host:true,tournamentPoints:state.wallet}]]);openHostPeer()}
