...
function renderWaiting(){
 ...
 box.innerHTML=`${spectatorView?'<div class="privacy-banner"><b>👀 観戦モード</b><br>所持ポイントが0ptのため馬券操作はできません。ほかの参加者の買い目を見ながら観戦できます。</div>':''}<b>${header}</b><br>${members.map(m=>`${isSpectatorMember(m)?'👀':validReady(m)?'✅':'⏳'} ${escapeHtml(m.horse||m.name||'参加者')}：${spectatorView?betDisclosure(m):(isSpectatorMember(m)?'観戦中':validReady(m)?'馬券確定':'選択中')}${m.host?'（主催者）':''}`).join('<br>')}`;
