...
function drawPreRaceGate(ctx,runners,open=0){
 const w=ctx.canvas.width,h=ctx.canvas.height;ctx.clearRect(0,0,w,h);const sky=ctx.createLinearGradient(0,0,0,h*.45);sky.addColorStop(0,'#82c9ff');sky.addColorStop(1,'#e8f5ff');ctx.fillStyle=sky;ctx.fillRect(0,0,w,h*.35);ctx.fillStyle='#1f7c42';ctx.fillRect(0,h*.35,w,h*.65);ctx.fillStyle='#c69a61';ctx.fillRect(0,h*.48,w,h*.47);
 ...
 const start=w*.16,goal=w*.93;ctx.fillStyle='#ffe34f';ctx.fillRect(start-5,h*.42,10,h*.55);for(let y=h*.42;y<h*.97;y+=20){ctx.fillStyle=((y/20)|0)%2?'#fff':'#111';ctx.fillRect(goal-7,y,14,20)}
 const gateX=start-w*.075,stallW=Math.max(26,w*.045),gateLeft=gateX-stallW*.6;ctx.fillStyle='#c7d0ca';ctx.fillRect(gateLeft,h*.46,stallW*1.2,h*.50);ctx.strokeStyle='#eef4f0';ctx.lineWidth=3;ctx.strokeRect(gateLeft,h*.46,stallW*1.2,h*.50);
 runners.forEach((r,i)=>{const laneY=h*.48+(i+.5)*(h*.47/runners.length);const horseX=gateX+open*w*.10;drawHorse(ctx,horseX,laneY,r,i);const doorShift=open*stallW*.65;ctx.strokeStyle='#7e8c84';ctx.lineWidth=Math.max(3,w*.006);for(let k=-2;k<=2;k++){const x=gateX+k*stallW*.12;ctx.beginPath();ctx.moveTo(x-doorShift,h*.46+i*0);ctx.lineTo(x-doorShift,h*.95);ctx.stroke();ctx.beginPath();ctx.moveTo(x+doorShift,h*.46);ctx.lineTo(x+doorShift,h*.95);ctx.stroke()}});
...
async function startCountdown(payload){
 ...
 if(!audioReady){
   $('commentary').textContent='音声を有効にしてファンファーレを始めてください。';
   $('leaderText').innerHTML='<button id="raceAudioStart" class="race-audio-start">🔊 ファンファーレ開始</button>';
   await new Promise(resolve=>{const b=$('raceAudioStart');if(!b){resolve();return}b.onclick=async()=>{await unlockAudio();resolve()}});
   if(token!==state.navToken)return;
 }
 $('leaderText').innerHTML='<span>ファンファーレ演奏中</span>';
 $('commentary').textContent='全馬ゲートイン。発走の瞬間を待ちます。';
 const played=await playRaceFanfare(state.race.name);
 if(token!==state.navToken)return;
 if(!played)await new Promise(r=>setTimeout(r,900));
...
function applyWhipEvent(data){if(!data?.tapId||state.whipSeen.has(data.tapId))return;state.whipSeen.add(data.tapId);const rt=state.raceRuntime;if(!rt)return;const r=rt.runners.find(x=>data.playerId?x.playerId===data.playerId:x.number===data.number);if(!r||r.finish||r.display<.76)return;r.whipTaps=(r.whipTaps||0)+1;const benefit=r.whipTaps<=8?.055:r.whipTaps<=12?.028:.008;r.finishWall=Math.max(rt.wall+.35,r.finishWall-benefit);r.whipFlash=performance.now()+180}
...
function runRace(payload,token=state.navToken){const canvas=$('track'),ctx=canvas.getContext('2d');resizeCanvas(canvas);const plan=officialPlan(payload.field,payload.seed),rnd=seeded(payload.seed),runners=plan.map((h,i)=>({...h,p:0,display:0,finish:false,lane:i,ph1:rnd()*7,ph2:rnd()*7,a1:.035+rnd()*.035,a2:.018+rnd()*.024,dramaPhase:rnd()*2,surgeAtA:.30+rnd()*.22,surgeAtB:.62+rnd()*.16,surgeA:(rnd()-.35)*.10,surgeB:(rnd()-.25)*.14,fadeAt:h.style==='逃げ'?.66:.79+rnd()*.07,fadePower:h.style==='逃げ'?.07+rnd()*.14:rnd()*.05,whipTaps:0,actualFinish:null}));state.whipSeen=new Set();state.raceRuntime={runners,wall:0};const dots=makeProgress(runners);$('liveTitle').textContent=state.race.name;$('liveCond').textContent=conditionText();const chosen=controlledRunner(runners);const banner=$('myHorseBanner');if(chosen&&(state.mode==='host'||state.mode==='join'||state.mode==='party')){banner.hidden=false;banner.innerHTML=`あなたが選んだのは <span style="background:${COLORS[(chosen.number-1)%COLORS.length]};color:${chosen.number===2?'#fff':'#111'}">${chosen.number}</span>番 <b>${escapeHtml(chosen.name)}</b>`}else banner.hidden=true;$('whipPanel').hidden=true;let t0=null,last=null,lastCall=-1,done=false;
 function frame(ts){if(token!==state.navToken)return;if(t0===null){t0=ts;last=ts}const wall=(ts-t0)/1000,dt=Math.min(.05,(ts-last)/1000);last=ts;state.raceRuntime.wall=wall;const leaderFinish=Math.min(...runners.map(r=>r.finishWall)),globalPhase=Math.max(0,Math.min(1,wall/leaderFinish));const liveFinishOrder=[...runners].sort((a,b)=>a.finishWall-b.finishWall||a.number-b.number),liveRank=new Map(liveFinishOrder.map((h,i)=>[h.number,i]));runners.forEach(h=>{if(h.finish){h.display=1;return}const t=Math.max(0,Math.min(1,wall/h.finishWall)),env=Math.sin(Math.PI*t),waves=(Math.sin(t*35+h.ph1)*h.a1+Math.sin(t*71+h.ph2)*h.a2)*env;const dramaFade=1-smooth((globalPhase-.66)/.20),style=(styleOffset(h.style,t)+raceDramaOffset(h,t))*dramaFade;const pack=smooth((globalPhase-.70)/.24),dynamicRank=liveRank.get(h.number)??h.finalRank;const rankSpread=((runners.length-1-dynamicRank)-(runners.length-1)/2)*.00105;const finalRush=smooth((globalPhase-.70)/.30);const packCenter=.74+.255*finalRush;const natural=t+waves+style;const packed=packCenter+rankSpread;let target=natural*(1-pack)+packed*pack;if(globalPhase>.94){const sprint=smooth((globalPhase-.94)/.06);target=target*(1-sprint)+(.988+rankSpread*.4)*sprint}target=Math.min(.9975,target);const follow=(target-h.display)*(1-Math.exp(-(9+pack*12)*dt)),minimum=dt/(h.finishWall*1.10);h.display=Math.min(.9975,Math.max(h.display+minimum,h.display+follow));if(wall>=h.finishWall){h.finish=true;h.actualFinish=h.finishWall}});if(globalPhase>=.975){const endEase=smooth((globalPhase-.975)/.025);runners.forEach(h=>{const rankGap=(h.finalRank||0)*.00085;h.display=Math.max(h.display,Math.min(.9995,1-rankGap-(1-endEase)*.0045))})}drawTrack(ctx,runners);updateProgress(dots,runners);const order=[...runners].sort((a,b)=>b.display-a.display),lead=order[0],control=controlledRunner(runners);$('clock').textContent=`0:${Math.min(32,wall).toFixed(1).padStart(4,'0')}`;$('remaining').textContent=`${Math.max(0,Math.round(state.race.distance*(1-lead.display)))}m`;const leaderColor=COLORS[(lead.number-1)%COLORS.length];$('leaderText').innerHTML=`<span class="leader-num" style="background:${leaderColor};color:${lead.number===2?'#fff':'#111'}">${lead.number}</span><span>先頭 ${escapeHtml(lead.name)}</span>`;const whipReady=(state.mode==='host'||state.mode==='join')&&control&&!control.finish&&control.display>=.76;$('whipPanel').hidden=!whipReady;if(whipReady)$('whipHorse').textContent=`${control.number}番 ${control.name}を追う！`;const c=Math.floor(wall/4);if(c!==lastCall){lastCall=c;$('commentary').textContent=commentary(c,order)}if(runners.every(h=>h.finish)||wall>32){if(!done){done=true;$('whipPanel').hidden=true;const sorted=[...runners].sort((a,b)=>(a.actualFinish??a.finishWall)-(b.actualFinish??b.finishWall)||a.number-b.number);const base=estimateSeconds(state.race.distance),first=sorted[0].actualFinish??sorted[0].finishWall;state.result=sorted.map(h=>({...h,officialTime:base+((h.actualFinish??h.finishWall)-first)*.42}));state.raceRuntime=null;setTimeout(showResult,700)}return}requestAnimationFrame(frame)}requestAnimationFrame(frame)}
...
function revealPartyRace(){state.field=state.partyEntries.map(partyRunner);calculateProbabilities();startCountdown({seed:state.seed,field:state.field,startAt:Date.now()+4200,config:raceConfig()})}
