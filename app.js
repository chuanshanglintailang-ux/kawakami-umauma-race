...
function officialPlan(field,seed){
 ...
 const base=27.2+rnd()*.25,gap=field.length>=10?.006:field.length>=7?.008:.010;
 return rows.map((x,rank)=>({...x.h,finalRank:rank,finishWall:base+rank*gap+rnd()*.0015}))
}
...
function runRace(payload,token=state.navToken){
 ...
 const raceDuration=Math.min(...runners.map(r=>r.finishWall));let sequenceStart=null,raceStart=null,lastCall=-1,done=false;
 function frame(ts){
  ...
  const wall=(ts-raceStart)/1000;state.raceRuntime.wall=wall;const phase=Math.max(0,Math.min(1,wall/raceDuration));
  runners.forEach(h=>{
   const style=styleOffset(h.style,phase);const wave=(Math.sin(phase*25+h.ph1)*.023+Math.sin(phase*47+h.ph2)*.012)*Math.sin(Math.PI*phase);
   const swap=h.swapPower1*Math.exp(-Math.pow((phase-h.swap1)/.085,2))+h.swapPower2*Math.exp(-Math.pow((phase-h.swap2)/.08,2));
   const compression=smooth((phase-.86)/.11),photo=smooth((phase-.955)/.045);const rankCenter=(runners.length-1)/2;const microscopic=(rankCenter-h.finalRank)*.00020;
   let target=phase+(style+wave+swap)*(1-compression);
   target=target*(1-compression)+(phase+microscopic)*compression;
   if(photo>0)target=target*(1-photo)+(.996+phase*.004+microscopic)*photo;
   // 全馬は必ず前進し続ける。停止や後退はさせない。
   const minAdvance=.000035;h.display=Math.min(.9997,Math.max(h.display+minAdvance,target));
   if(wall>=h.finishWall){h.finish=true;h.actualFinish=h.finishWall;h.display=.99962-h.finalRank*.00008}
  });
